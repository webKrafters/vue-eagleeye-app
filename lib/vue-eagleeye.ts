...
> implements Store<T, S> {
	...
	constructor( stream : BaseStream<T>, selectorMap : S ) {
		...
		this._channel.addListener( 'data-changed', () => this.updateData() );
		onBeforeUnmount(() => this._channel.endStream());
		this.updateData();
	}
	...
	get resetState() { return this._channel.resetState.bind( this._channel ) }
	
	get setState() { return this._channel.setState.bind( this._channel ) }
...
> {
	private _channel : BaseChannel<T, S>;
	private _data = reactive({}) as Reactive<Data<S, T>>;
	private _store : VueStore<T,S>;
	constructor( stream : BaseStream<T>, selectorMap : S ) {
		this._channel = stream( selectorMap );
		{
			const ctx = this;
			this._store = {
				get data() { return ctx._data },
				set selectorMap( selectorMap : S ) {
					ctx._channel.selectorMap = selectorMap;
				},
				resetState: ctx._resetState.bind( ctx ),
				setState: ctx._setState.bind( ctx ),
			};
		}
		...
		onUnmounted(() => {
			...
			this._channel = null as unknown as BaseChannel<T,S>;
			this._store = null as unknown as VueStore<T, S>;
		});
		this.updateData();
	}
	...
	private updateData() {
		...
		for( const k in currFieldMap ) {
			delete currFieldMap[ k as string ];
		}
	}
	private getCurrStateFieldMap() {
